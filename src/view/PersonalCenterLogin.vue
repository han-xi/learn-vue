      ...
      <div id="register">
        ...
        >
          ...
          <el-form-item prop="mail"  >
            <el-input v-model="Login.mail" prefix-icon="el-icon-user-solid" auto-complete="off" placeholder="请输入邮箱地址"></el-input>
          </el-form-item>
          <!-- 密码 -->
          <el-form-item prop="pass">
            <el-input type="password" v-model="Login.password" prefix-icon="el-icon-lock" auto-complete="off" placeholder="输入密码"></el-input>
          </el-form-item>
          <!-- 验证码 -->
          <el-form-item prop="info">
            <el-input  v-model="info" prefix-icon="el-icon-key" auto-complete="off" placeholder="请输入验证码"></el-input>
            <el-span class="code-style" @click="createCode">{{vercode}}</el-span>
          </el-form-item>
          <!-- 登录 -->
          <el-form-item>
          <el-button type="primary" @click="login('Login')" :loading="flag" style="width:100%;">登录</el-button>
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {//如果表单格式不正确直接跳转登录失败，不发送信息给后台
          if(!this.confirmTheCode()){//如果验证码不正确显示验证失败信息
            ...
            this.flag=true
              //发送用户名和密码到后端
              user.login(this.Login.mail,this.Login.password).then(response=>{
                if(!window.localStorage){
                   swal("浏览器不支持localstorage");
                  }
                else{
...
<style scoped>
#icon1{
background-image: url(../assets/icon/username.png);/*设置小图标为背景*/
background-size:100px 100px;/*设置小图标的 大小*/
background-position: 5px 4px;/*设置小图标的位置*/
